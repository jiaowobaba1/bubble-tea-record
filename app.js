...
const flavors = {
  '喜茶': ['多肉葡萄', '芝芝莓莓', '烤黑糖波波牛乳', '纯绿妍茶', '芝芝芒芒', '满杯红柚', '多肉桃桃'],
...
document.querySelectorAll('.choice-group').forEach((group) => group.addEventListener('click', (event) => { if (event.target.tagName !== 'BUTTON') return; group.querySelectorAll('button').forEach((button) => button.classList.remove('selected')); event.target.classList.add('selected'); }));
 $('brand').addEventListener('change', (event) => { const isOtherBrand = event.target.value === 'other'; const options = flavors[event.target.value] || (isOtherBrand ? generalFlavors : []); $('customBrandLabel').classList.toggle('hidden', !isOtherBrand); $('customBrand').required = isOtherBrand; $('flavor').innerHTML = options.length ? `<option value="">请选择口味</option>${options.map((item) => `<option>${item}</option>`).join('')}<option value="other">其他</option>` : '<option value="">先选择品牌</option>'; $('flavor').disabled = options.length === 0; $('customFlavorLabel').classList.add('hidden'); $('customFlavor').required = false; });
